---
name: insight
description: 当任务涉及归因分析、业务诊断、活动复盘、营销分析、渠道/用户/商品/优惠/地域/漏斗分析，或需要先通过 `db_query` 查询业务数据库、再用 pandas 在工作区完成多维对比分析并最终输出 HTML 报告时使用此技能。不要只返回单次查询结果；默认要做多角度、多维度归因拆解，沉淀可复用数据文件、中间结果和最终 HTML 页面。工作区 Python 环境使用 uv 管理，执行脚本、单文件命令或安装依赖时必须使用 `uv run` 或 `uv add`。
---
