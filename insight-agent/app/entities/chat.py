...
class ContextCompaction(Base):
    ...
    end_seq: Mapped[int] = mapped_column(
        BigInteger, nullable=False, comment="本次压缩覆盖的结束上下文顺序号(0-based, 不包含)"
    )
...
class Message(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["conversation_id"], ["conversation.id"], ondelete="CASCADE", name="message_ibfk_1"
        ),
        Index("idx_message_conversation_id", "conversation_id"),
        Index(
            "uk_message_conversation_id_context_seq", "conversation_id", "context_seq", unique=True
        ),
        {"comment": "消息"},
    )
