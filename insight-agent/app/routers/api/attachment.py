...
async def api_upload_attachment(
    ...
    # 获取文件名
    f_name = file.filename or "upload"
    # 构造文件路径
    workspace_dir = get_workspace_dir(user_id, conversation_id)
    target_path = _build_attachment_path(workspace_dir, f_name)
    # 按块读取并落盘，避免一次性把整个文件读入内存
    with target_path.open("wb") as target_file:
        while chunk := await file.read(1024 * 1024):
            target_file.write(chunk)

    logger.info(f"Upload attachment: {conversation_id=}, file={f_name}")
    return chat_schema.UploadAttachmentResponse(
        attachment=chat_schema.Attachment(f_name=f_name)
    )
...
async def api_delete_attachment(
    ...
    # 获取附件文件路径
    target_path = _build_attachment_path(workspace_dir, body.f_name)
    # 删除文件
    if target_path.exists() and target_path.is_file():
        target_path.unlink()

    logger.info(
        f"Delete attachment: conversation_id={body.conversation_id}, file={body.f_name}"
    )
...
async def api_get_attachment(
    ...
    logger.info(f"Get attachment: {conversation_id=}, file={f_name}")
    return FileResponse(
        path=target_path,
