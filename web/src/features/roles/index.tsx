...
import { AssetPermissionPanel } from "@/features/roles/AssetPermissionPanel";
...
export function DorisRoleManagement() {
  ...
  const loadRoles = useCallback(async () => {
    ...
    try {
      const [loadedRoles, loadedExistingRoles, loadedWorkloadGroups] = await Promise.all([
        rolesApi.listRoles(),
        rolesApi.listExistingRoles(),
        rolesApi.listWorkloadGroups(),
      ]);
  ...
  useEffect(() => {
    ...
    void Promise.all([
      rolesApi.listRowPolicies(selectedRole),
      rolesApi.listSelectGrants(selectedRole),
    ])
                        ...
                        <Button
                          ...
                          onClick={(e) => {
                            ...
                            void mutate(
                              () =>
                                role.is_default
                                  ? rolesApi.clearDefaultRole()
                                  : rolesApi.setDefaultRole(role.name).then(() => undefined),
