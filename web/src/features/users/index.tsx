...
import { rolesApi, type DorisRoleResponse } from "@/features/roles/api";
import { usersApi, type UserListResponse } from "@/features/users/api";
import { getApiErrorMessage } from "@/api/errors";
import { type UserResponse, useAuthStore } from "@/auth/index";
...
export function UserManagement() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [loadedRoles, userPage] = await Promise.all([
        rolesApi.listRoles(),
        usersApi.listUsers(USER_PAGE_SIZE, userOffset, searchQuery),
      ]);
  ...
  const handleDeleteUser = async (user: UserResponse) => {
    ...
    try {
      ...
      const page = await usersApi.listUsers(USER_PAGE_SIZE, nextOffset, searchQuery);
  ...
  const reloadFirstPage = async () => {
    const page = await usersApi.listUsers(USER_PAGE_SIZE, 0, searchQuery);
    setUserOffset(0);
    applyUserPage(page);
  };
