...
平台管理员负责用户、角色、元数据和查询经验管理；普通用户负责对话和分析。管理员身份只赋予平台管理能力，查询业务数据仍需要绑定 Doris 角色并遵守该角色的权限。
...
- **SELECT 资产权限管理**
  ...
  - 查询经验和召回缓存使用角色与权限指纹限定作用范围，权限变化后旧范围的结果不能继续复用。
...
- **表元数据管理**
  ...
  - 提交目录变更后交给 `MetadataChangeWorkflow` 处理受影响的查询经验和索引任务。
...
- **资源删除与依赖保护**
  ...
  - 表或字段删除后，使受影响的查询经验失效；指标变更更新其语义索引，查询经验的资产引用按表和字段校验。
...
- **Conversation 查询上下文**
  ...
  - 查询经验缓存 1 天；角色或权限指纹变化后不再复用该缓存，后续召回重新检索。
  - 将查询经验与语义召回结果共同保存为快照。
...
- **查询经验生成**
  ...
  - 按角色、`authorization_fingerprint` 和 SQL 模板 `fingerprint` 创建或更新经验；权限指纹与 SQL 指纹是两个不同维度。
  - 保存最近的去重查询目的。
...
- **元数据变更编排**
  ...
  - 变更描述区分需要失效的表、字段，以及需要重新索引的字段、指标；删除资产只参与失效。
  - `MetadataChangeWorkflow` 先调用 Query 使旧经验失效，再依次提交字段和指标索引任务。
  - `workflows/providers.py` 为 HTTP 和 Worker 组装同一套跨模块能力。
  - 工作流不拥有目录事务、不吞掉后续失败，也没有把目录和消息队列合并为原子事务。
  - 若失效或投递失败，目录可能已经提交；排查时需分别核对目录版本、经验状态和已提交的索引任务。
...
| Metadata 目录/导入服务   | 注入的 `MetadataChangeWorkflow`                   | 提交后失效旧经验、调度新索引，服务不自行组装 Query |
| Query Runtime            | Identity                                          | 同一次授权观察获得查询凭据和不可变资产策略         |
| Query Guard / 经验服务   | Metadata                                          | 校验表字段、类型与关联，保存并检查资产版本         |
| Query 执行器             | Sandbox Manager | 流式生成 CSV 后提交到 Explorer Session             |
| Assistant 召回 Runtime   | Identity、Metadata、Query                         | 重新授权，合并业务资源与历史查询经验               |
| Assistant 对话与执行服务 | Sandbox                                           | 上传资料、执行分析、核验产物及清理工作区           |
| MetadataChangeWorkflow   | Query、Metadata 调度器                            | 先处理经验失效，再提交字段/指标索引                |
| UserDeletionService      | Identity、Assistant、Sandbox                      | 按会话资源 → 用户沙箱 → 用户身份顺序完成注销       |

Metadata 与 Query 共用 `meta` 数据库，但各自维护自己的模型和仓储。Query 需要读取目录；目录变更引起的 Query 经验失效由 Workflows 协调。共享数据库不意味着共享业务事务，也不意味着任意模块可以直接修改对方的表。
...
| `/api/v1/admin/query-experiences` | `query/api/admin`                   | 管理员查询、禁用和删除经验，查看来源执行记录          |
...
| ES 搜索文档                                       | Elasticsearch         | 可由目录和经验重建，可能暂时落后于数据库版本             |
...
4. **业务召回。** Explorer 按稳定 query 检索字段、指标、字段值和历史 SQL 经验。实时权限约束本次结果，快照引用在模型调用前临时展开。
5. **受控查数。** Query 解析角色查询身份，Guard 校验 SQL 和当前目录，Doris 按查询账号、行策略、工作组及资源限制执行。
6. **结果交接。** 全量结果流式写入 Explorer 的 CSV；模型接收路径、行数、字段与少量样例。执行记录和经验按独立事务保存，索引在后台更新。
...
sequenceDiagram
    ...
    participant M as Metadata / Query 经验
...
管理员修改或导入目录 → Metadata 校验 Doris 物理结构与目录依赖 → 提交 PostgreSQL 目录事务 → Workflows 使受影响的表/字段经验失效 → 提交语义索引任务 → Worker 计算文本差量与向量 → 写入 ES → 核对版本并更新索引状态。
...
| Celery Worker                   | 元数据导入/索引、查询经验索引、标题生成、会话与用户资源清理     |
...
字段取值调度、过期草稿及待删除会话清理、到期注销任务领取、查询经验索引修复由 Beat 周期触发。标题任务使用当前配置的主模型，`lightweight` 是队列名称，不表示另有一套小模型配置。
...
| 查询经验       | 角色、权限指纹、资产版本和状态共同校验，索引可修复                        | 历史 SQL 只是参考，复用后仍要重新经过 Guard 和 Doris 执行                     |
