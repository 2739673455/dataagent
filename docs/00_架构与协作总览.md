...
flowchart TD
    ...
    E --> RECALL["语义召回工具<br/>recall_context"]
    ...
    M -->|元数据目录| META_PG[(元数据 PostgreSQL)]
...
平台管理员负责用户和角色管理；普通用户负责对话和分析。管理员身份只赋予平台管理能力，查询业务数据仍需要绑定 Doris 角色并遵守该角色的权限。
...
### 3.3 `metadata`：元数据导入与语义召回

`metadata` 通过脚本维护表、字段和指标目录及检索索引，并为 Explorer 构建持续可用的查询上下文。

- **全量导入**

  - 读取 YAML，校验格式、名称唯一性、字段引用和指标依赖。
  - 检查 Doris 物理表、字段及水位列，读取主键、字段类型和有限示例值。
  - 检查通过后删除并重建三个元数据索引，清空目录、关联、取值同步状态。
  - 写入新目录，顺序完成字段语义、指标语义和字段取值索引。
  - 全量过程失败后重跑全量脚本，不提供合并、预览或单项管理入口。

- **字段取值增量导入**

  - 从已导入目录读取水位配置，每张表读取一次最大水位。
  - 只处理启用取值索引的字段；未配置水位的表跳过。
  - 水位未推进时不扫描，有新值时读取 `(上次水位, 本次上界]`。
  - 全量时空表可以在出现数据后从无下界开始增量导入。
  - 索引写入成功后提交水位；失败保留旧水位，重试按稳定文档 ID 去重。
  - 同水位或旧水位迟到数据、源数据删除通过全量重建处理。

- **脚本资源与索引**

  - 全量和增量模式互斥运行，进程退出后释放数据库锁。
  - 脚本直接完成导入和索引构建，不依赖 HTTP、Worker 或 Beat。
  - 名称、描述和别名生成字段及指标语义文档；字段取值使用全文索引。
  - 向量模型输出维度必须与 `elasticsearch.embedding_size` 一致。
  - PostgreSQL 与 Elasticsearch 没有跨库原子事务，日志和字段水位用于判断进度。
...
- **模型可见结果**
  ...
  - Explorer 的 `recall_context` 直接返回本次授权后的表、字段、值和指标。
  - 结果正文保留在工具消息中，后续轮次持续可见，不再折叠或按引用展开。
  - 实时事件和历史展示直接读取消息正文，不回查召回数据库。
  - 不维护 query 累计上下文、召回快照或记录管理工具。
  - 排名、匹配原因、索引状态和失败范围保留在服务内部。
...
`query` 负责安全执行 Explorer 提交的 SQL，交付完整查询结果。
...
- **查询身份解析**
  ...
  - 解密角色查询密码，由 Doris 校验 SQL 的实际访问权限。
  - 在进入后续阶段前关闭身份数据库会话。
  - 身份解析完成后关闭认证会话；Guard 在本地检查语法，不读取元数据库。

- **SQL 静态 Guard**

  - 使用 Doris 方言解析单条 SQL。
  - 业务查询只允许 `SELECT` 或最终返回 `SELECT` 的 `WITH`。
  - 数据发现阶段允许 `SHOW TABLES`，可见范围由 Doris 控制。
  - `information_schema` 查询使用与业务查询相同的只读检查。
  - 拒绝 DDL、DML、其他命令和不安全函数。
  - 表、字段、星号展开及 JOIN 语义交给 Doris 处理。

- **Doris 受限执行**

  - 使用角色专用连接执行查询。
  - 使用 Doris 自身的 Workload Group、查询超时和内存配置。
  - 通过服务端游标分批读取结果。
  - 持续校验每批列名与结果形状一致。
...
- **Conversation 清理**
  ...
  - 删除沙箱目录和 Conversation 记录。
  - 周期扫描恢复丢失的删除任务。
  - 周期任务清理过期草稿。

### 3.7 `workflows`：跨模块变更与生命周期工作流

`workflows` 负责把多个模块的公开能力按业务顺序组合起来。用户注销通过持久化任务记录和 Celery 执行可重试清理。
...
| 元数据 PostgreSQL（`meta`）     | `metadata` 的表、字段、指标、索引状态                                     |
...
| ES 搜索文档                                       | Elasticsearch         | 可由目录重建，重建中可能尚未全部就绪             |
...
4. **业务召回。** Explorer 检索字段、指标和字段值。实时权限约束本次结果，工具消息直接保存结果正文，后续轮次持续可见。
5. **受控查数。** Query 解析角色查询身份，Guard 检查只读语法和危险操作，Doris 按查询账号、行策略、工作组及资源限制执行。
6. **结果交接。** 全量结果流式写入 Explorer 的 CSV；模型接收路径、行数、字段与少量样例。
...
全量脚本校验 YAML 和源表后，清空重建目录与三个检索索引；后续增量脚本按表水位追加字段取值。索引写入成功后提交水位，失败保留已完成阶段供排查。目录配置变更、迟到数据或源数据删除通过重新全量导入处理。
...
| 删除 Conversation | 取消本进程 Run，取得生命周期锁并标记删除，后台清理 Checkpoint、会话目录及记录，墓碑与周期扫描支撑重试 |
...
对话 Run 在 API 进程执行，不经过 Celery。`lightweight` 承载标题生成；`lifecycle` 承载其余 Assistant 与 Workflows 任务；未匹配任务进入 `default`。Worker 需消费相应队列，单独启动 API 不会替代 Worker 和 Beat。
...
过期草稿及待删除会话清理、到期注销任务领取由 Beat 周期触发。标题任务使用当前配置的主模型，`lightweight` 是队列名称，不表示另有一套小模型配置。
...
| 目录和搜索索引 | 全量重建、取值水位、generation 和失败状态                                 | PostgreSQL 与 ES 没有跨库事务，失败时需检查具体完成阶段                       |
