...
模型上下文会在调用前补充附件和召回资料。Checkpoint 尽量保存引用，避免把每次展开的大段资料重复写进历史。语义召回引用在再次展开时按当前权限过滤。旧会话中的召回引用不能保留已撤销的访问权限。
...
Analyst 挂载分析与可视化 Skill；技能目录只读。用户附件中间件在调用前准备当前可用输入，语义召回中间件临时展开本回合引用，时间戳中间件维护消息展示时间。内部工具记录、公开活动和最终用户消息按各自协议投影。
