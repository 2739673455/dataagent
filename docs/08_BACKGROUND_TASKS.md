...
[`docker/compose.yml`](../docker/compose.yml) 使用 `redis:8.10.0-alpine`，开启 AOF 持久化并挂载 `redis-data` 数据卷。任务结果默认保留 24 小时，业务事实、同步版本、语义召回快照和删除墓碑保存在 PostgreSQL 关系表中。
...
Worker 消费全部业务队列，并且需要访问与 API 相同的 Docker Engine 和沙箱数据卷。Beat 只能运行一个实例，避免同一周期任务被重复发布。
