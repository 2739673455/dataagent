...
class TestAuthAPIBasic:
    ...
    async def test_register_success(self, async_client, test_user_data):
        """测试注册成功"""
        response = await async_client.post("/api/register", json=test_user_data)
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_register_invalid_email(self, async_client):
        ...
        response = await async_client.post("/api/register", json=invalid_data)
    ...
    @pytest.mark.asyncio
    async def test_login_success(self, async_client, test_user_data3):
        """测试登录成功"""
        # 先注册
        await async_client.post("/api/register", json=test_user_data3)
        # 登录
        login_data = {
    ...
    @pytest.mark.asyncio
    async def test_update_username_without_token(self, async_client):
        """测试未携带令牌修改用户名"""
        response = await async_client.post("/api/me/username", json={"username": "newname"})
        assert response.status_code == 401

    @pytest.mark.asyncio
    async def test_get_user_info(self, async_client, test_user_data4):
        """测试获取用户信息"""
        # 先注册
        register_response = await async_client.post("/api/register", json=test_user_data4)
    ...
    @pytest.mark.asyncio
    async def test_update_username(self, async_client, test_user_data5):
        """测试修改用户名"""
        # 先注册
        register_response = await async_client.post("/api/register", json=test_user_data5)
        tokens = register_response.json()
        access_token = tokens["access_token"]
