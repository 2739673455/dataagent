...
export const permissionRoleApi = {
	...
	updateRole: (data: UpdateRoleRequest) =>
		apiClient.post<RoleInfo>("/api/admin/update_role", data),

	// 删除角色
	removeRole: (data: RemoveRoleRequest) =>
		apiClient.post<void>("/api/admin/remove_role", data),
	...
	getRole: (roleId: number) =>
		apiClient.get<RoleDetailResponse>(`/api/admin/role/${roleId}`),
};
...
export const permissionApi = {
	...
	listPermissions: (params: {
		...
		keyword?: string;
		all?: boolean;
	}) => apiClient.get<PermissionListResponse>("/api/admin/list_permissions", { params }),
