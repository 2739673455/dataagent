...
def _setup_logger(cfg: LogCfg, logger_name: str, processors: list):
    ...
    handlers = []
    if cfg.to_console:
        handlers.append(_make_console_handler(cfg, processors))

    if cfg.to_file:
        handlers.append(_make_file_handler(cfg, processors))
...
def stop_background_logging() -> None:
    """停止后台日志处理线程并清理"""
    global _queue_listener, _log_queue
    if _queue_listener:
