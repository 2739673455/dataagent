...
class Conversation(Base):
    ...
    __table_args__ = (
        ForeignKeyConstraint(
            ["model_config_id"], ["model_config.id"], name="conversation_ibfk_1"
        ),
...
class Message(Base):
    ...
    role: Mapped[str] = mapped_column(
        String(20), nullable=False, comment="角色 (user/assistant/tool)"
    )
    content: Mapped[str] = mapped_column(
        Text, nullable=False, comment="消息内容 (JSON 字符串)"
    )
