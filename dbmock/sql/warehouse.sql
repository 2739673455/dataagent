...
CREATE TABLE `dwd_dim_user_info_df` (
    ...
    `age` INT DEFAULT NULL COMMENT '年龄(ETL衍生)',
    ...
    `user_level` TINYINT DEFAULT 1 COMMENT '会员等级',
    `user_tag` VARCHAR(128) DEFAULT NULL COMMENT '用户标签',
    `is_new_user` TINYINT DEFAULT 0 COMMENT '是否新客:0否 1是',
    ...
    `user_status` TINYINT DEFAULT 1 COMMENT '状态:0禁用 1正常 2注销',
    `first_order_time` DATETIME DEFAULT NULL COMMENT '首单时间',
    `last_order_time` DATETIME DEFAULT NULL COMMENT '最近下单时间',
    `etl_date` DATE NOT NULL COMMENT '分区日期(yyyy-mm-dd)',
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP COMMENT '入仓时间',
    `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_user_etl` (`user_id`, `etl_date`),
    KEY `idx_province_city` (`province_code`, `city_code`),
    KEY `idx_register_time` (`register_time`),
    KEY `idx_last_order_time` (`last_order_time`)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT = '用户维度明细快照';
...
CREATE TABLE `dwd_dim_shop_info_df` (
    ...
    `shop_type` TINYINT DEFAULT 1 COMMENT '店铺类型:1自营 2旗舰店 3专卖店 4普通店',
    ...
    `shop_status` TINYINT DEFAULT 1 COMMENT '状态:0关店 1营业',
...
CREATE TABLE `dwd_dim_category_info_df` (
    ...
    `category_level` TINYINT NOT NULL COMMENT '层级:1/2/3',
...
CREATE TABLE `dwd_dim_spu_info_df` (
    ...
    `shelf_status` TINYINT DEFAULT 1 COMMENT '上下架状态:1上架 2下架',
    `on_shelf_time` DATETIME DEFAULT NULL COMMENT '上架时间',
    `off_shelf_time` DATETIME DEFAULT NULL COMMENT '下架时间',
    `sale_num` BIGINT DEFAULT 0 COMMENT '累计销量',
    `comment_num` BIGINT DEFAULT 0 COMMENT '累计评价数',
    `good_comment_rate` DECIMAL(5, 2) DEFAULT NULL COMMENT '好评率(%)',
    `etl_date` DATE NOT NULL COMMENT '分区日期',
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_spu_etl` (`spu_id`, `etl_date`),
    KEY `idx_shop_id` (`shop_id`),
    KEY `idx_category_id` (`category_id`),
    KEY `idx_brand_id` (`brand_id`)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT = 'SPU维度明细快照';
...
CREATE TABLE `dwd_dim_sku_info_df` (
    ...
    `shelf_status` TINYINT DEFAULT 1 COMMENT '上架状态:1上架 2下架',
    `etl_date` DATE NOT NULL COMMENT '分区日期',
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_sku_etl` (`sku_id`, `etl_date`),
    KEY `idx_spu_id` (`spu_id`),
    KEY `idx_shop_id` (`shop_id`),
    KEY `idx_category_id` (`category_id`),
    KEY `idx_brand_id` (`brand_id`)
) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COMMENT = 'SKU维度明细快照';
...
CREATE TABLE `dwd_dim_coupon_info_df` (
    ...
    `received_cnt` BIGINT DEFAULT NULL COMMENT '已领取数量',
    `used_cnt` BIGINT DEFAULT NULL COMMENT '已使用数量',
    `status` TINYINT DEFAULT 1 COMMENT '状态:1有效 0失效',
    `etl_date` DATE NOT NULL COMMENT '分区日期',
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE `dwd_dim_geo_region_df` (
    ...
    `is_hot_city` TINYINT DEFAULT 0 COMMENT '是否热门城市',
    `status` TINYINT DEFAULT 1 COMMENT '状态:1有效 0失效',
    `etl_date` DATE NOT NULL COMMENT '分区日期',
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
    `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
...
CREATE TABLE `dwd_fact_trade_order_detail_di` (
    ...
    `platform_discount_amount` DECIMAL(16, 2) DEFAULT 0 COMMENT '平台优惠金额',
    `shop_discount_amount` DECIMAL(16, 2) DEFAULT 0 COMMENT '店铺优惠金额',
    ...
    `gross_profit_amount` DECIMAL(16, 2) DEFAULT 0 COMMENT '毛利金额(ETL衍生)',
    `order_create_time` DATETIME NOT NULL COMMENT '下单时间',
    `order_confirm_time` DATETIME DEFAULT NULL COMMENT '确认时间',
...
CREATE TABLE `dwd_fact_trade_pay_detail_di` (
    ...
    `pay_status` VARCHAR(32) NOT NULL COMMENT '支付状态:成功/失败/关闭/退款',
...
CREATE TABLE `dwd_fact_service_comment_detail_di` (
    ...
    `is_positive` TINYINT DEFAULT NULL COMMENT '是否好评',
