...
class DwdDimCategoryInfoDf(Base):
    ...
    category_level: Mapped[int] = mapped_column(TINYINT, nullable=False, comment='层级:1/2/3')
...
class DwdDimGeoRegionDf(Base):
    ...
    is_hot_city: Mapped[Optional[int]] = mapped_column(TINYINT, server_default=text("'0'"), comment='是否热门城市')
    status: Mapped[Optional[int]] = mapped_column(TINYINT, server_default=text("'1'"), comment='状态:1有效 0失效')
    created_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('CURRENT_TIMESTAMP'))
    updated_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'))
...
class DwdDimShopInfoDf(Base):
    ...
    etl_date: Mapped[datetime.date] = mapped_column(Date, nullable=False, comment='分区日期')
    shop_type: Mapped[Optional[int]] = mapped_column(TINYINT, server_default=text("'1'"), comment='店铺类型:1自营 2旗舰店 3专卖店 4普通店')
    ...
    shop_status: Mapped[Optional[int]] = mapped_column(TINYINT, server_default=text("'1'"), comment='状态:0关店 1营业')
    created_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('CURRENT_TIMESTAMP'))
    updated_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'))
...
class DwdDimSpuInfoDf(Base):
    ...
    sale_num: Mapped[Optional[int]] = mapped_column(BigInteger, server_default=text("'0'"), comment='累计销量')
    comment_num: Mapped[Optional[int]] = mapped_column(BigInteger, server_default=text("'0'"), comment='累计评价数')
    good_comment_rate: Mapped[Optional[decimal.Decimal]] = mapped_column(DECIMAL(5, 2), comment='好评率(%)')
    created_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('CURRENT_TIMESTAMP'))
    updated_at: Mapped[Optional[datetime.datetime]] = mapped_column(DateTime, server_default=text('CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP'))
...
class DwdDimUserInfoDf(Base):
    ...
    gender: Mapped[Optional[int]] = mapped_column(TINYINT, server_default=text("'0'"), comment='性别:0未知 1男 2女')
    birthday: Mapped[Optional[datetime.date]] = mapped_column(Date, comment='生日')
    age: Mapped[Optional[int]] = mapped_column(Integer, comment='年龄(ETL衍生)')
    ...
    user_level: Mapped[Optional[int]] = mapped_column(TINYINT, server_default=text("'1'"), comment='会员等级')
    user_tag: Mapped[Optional[str]] = mapped_column(String(128), comment='用户标签')
    is_new_user: Mapped[Optional[int]] = mapped_column(TINYINT, server_default=text("'0'"), comment='是否新客:0否 1是')
    ...
    user_status: Mapped[Optional[int]] = mapped_column(TINYINT, server_default=text("'1'"), comment='状态:0禁用 1正常 2注销')
...
class DwdFactServiceCommentDetailDi(Base):
    ...
    etl_date: Mapped[datetime.date] = mapped_column(Date, nullable=False, comment='分区日期')
    category_id: Mapped[Optional[int]] = mapped_column(BIGINT(unsigned=True), comment='类目ID')
    is_positive: Mapped[Optional[int]] = mapped_column(TINYINT, comment='是否好评')
...
class DwdFactTradeOrderDetailDi(Base):
    ...
    gross_profit_amount: Mapped[Optional[decimal.Decimal]] = mapped_column(DECIMAL(16, 2), server_default=text("'0.00'"), comment='毛利金额(ETL衍生)')
